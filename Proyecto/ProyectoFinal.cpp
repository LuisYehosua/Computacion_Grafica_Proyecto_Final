...
float adventurerRotY = 0.0f;    // rotacion en Y (grados)
...
void loadFromFileRobot(const char* filename)
{
	...
	file.close();
	printf("Animacion robot cargada desde %s (%d keyframes)\n", filename, frameIndexRobot);
}
...
void AnimationPerro()
{
	...
	if (iCurrStepsPerro >= iMaxStepsPerro)
	{
		...
		{
			printf("Perro - animacion terminada\n");
			playIndexPerro = 0;
			playPerro = false;
		}
		else
...
void loadFromFilePerro(const char* filename)
{
	...
	file.close();
	printf("Animacion perro cargada desde %s (%d keyframes)\n", filename, frameIndexPerro);
}
...
void AnimationMujer()
{
	...
	if (iCurrStepsMujer >= iMaxStepsMujer)
	{
		...
		{
			printf("Mujer - animacion terminada\n"); playMujer = false; playIndexMujer = 0;
		}
		else
		{
...
void loadFromFileMujer(const char* filename)
{
	...
	file.close();
	printf("Animacion mujer cargada desde %s (%d keyframes)\n", filename, frameIndexMujer);
}
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (adventurerActive)
		{
			...
			if (!adventurerTurned)
			{
				// Fase 1: corre hacia adelante (Z positivo)
				adventurerZ += deltaTime * adventurerSpeed;

				// Cuando llega a Z = 16, gira (mas adelante para no chocar)
				if (adventurerZ >= 13.0f)
				{
					adventurerTurned = true;
					adventurerRotY = 90.0f;
				}
			}
			else
			{
				// Fase 2: corre hacia X positivo (donde apunta ahora)
				adventurerX += deltaTime * adventurerSpeed;

				// Avanza un poquito, se detiene, desaparece y regresa al inicio
				if (adventurerX > 22.0f)
				{
					adventurerActive = false;
					adventurerInvisible = true;
					// Regresar a punto inicial (invisible, listo para volver con T)
					adventurerX = 17.0f;
					adventurerZ = -10.0f;
					adventurerRotY = 0.0f;
					adventurerTurned = false;
					printf("Aventurero - llego al final y regreso al inicio\n");
				}
			}
		}
	...
	/*	model = modelTemp;
		...
		if (!adventurerInvisible)
		{
			...
			glUniformMatrix4fv(glGetUniformLocation(animShader.Program, "projection"),
				1, GL_FALSE, glm::value_ptr(projection));
			glUniformMatrix4fv(glGetUniformLocation(animShader.Program, "view"),
				1, GL_FALSE, glm::value_ptr(viewAnim));

			// posicion del personaje (X y Z dinamicos, rotacion segun fase)
			glm::mat4 modelAnim = glm::mat4(1.0f);
			modelAnim = glm::translate(modelAnim, glm::vec3(adventurerX, sceneRiseY, adventurerZ));
			modelAnim = glm::rotate(modelAnim, glm::radians(adventurerRotY), glm::vec3(0.0f, 1.0f, 0.0f));
			modelAnim = glm::scale(modelAnim, glm::vec3(0.035f));
			glUniformMatrix4fv(glGetUniformLocation(animShader.Program, "model"),
				1, GL_FALSE, glm::value_ptr(modelAnim));

			// matrices de huesos
			auto transforms = animator.GetFinalBoneMatrices();
			for (int i = 0; i < (int)transforms.size(); ++i) {
				std::string name = "finalBonesMatrices[" + std::to_string(i) + "]";
				glUniformMatrix4fv(glGetUniformLocation(animShader.Program, name.c_str()),
		...
		if (startSceneRise)
		{
			...
			glm::mat4 viewTib = camera.GetViewMatrix();
			glUniformMatrix4fv(glGetUniformLocation(animShader.Program, "projection"),
				1, GL_FALSE, glm::value_ptr(projection));
			glUniformMatrix4fv(glGetUniformLocation(animShader.Program, "view"),
				1, GL_FALSE, glm::value_ptr(viewTib));

			// Tiburon bailarin: sube con R desde abajo, queda a nivel del suelo
			glm::mat4 modelTib = glm::mat4(1.0f);
			modelTib = glm::translate(modelTib, glm::vec3(0.0f, sceneRiseY, 0.0f));    // animacion de subida
			modelTib = glm::translate(modelTib, glm::vec3(22.0f, 0.0f, 6.0f));         // posicion donde estaba el estatico
			modelTib = glm::rotate(modelTib, glm::radians(-90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
			modelTib = glm::scale(modelTib, glm::vec3(0.010f));                        // mas chiquito
			glUniformMatrix4fv(glGetUniformLocation(animShader.Program, "model"),
				1, GL_FALSE, glm::value_ptr(modelTib));

			auto transformsTib = animatorTiburon.GetFinalBoneMatrices();
			for (int i = 0; i < (int)transformsTib.size(); ++i) {
				std::string name = "finalBonesMatrices[" + std::to_string(i) + "]";
				glUniformMatrix4fv(glGetUniformLocation(animShader.Program, name.c_str()),
					1, GL_FALSE, glm::value_ptr(transformsTib[i]));
			}
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	// R  -> cargar animaciones y suben stands
	...
	if (key == GLFW_KEY_Q && action == GLFW_PRESS)
	{
		if (!playRobot && frameIndexRobot > 1)
		{
			...
			playIndexRobot = 0;
			iCurrStepsRobot = 0;
			printf("Robot - reproduciendo animacion\n");
		}
		else
		{
			playRobot = false;
			printf("Robot - animacion detenida\n");
		}
	}
	...
	if (key == GLFW_KEY_T && action == GLFW_PRESS)
	{
		if (adventurerActive)
		{
			// Si esta corriendo, lo detenemos en su lugar
			adventurerActive = false;
			printf("Aventurero - detenido\n");
		}
		else
		{
			// Si esta detenido, reinicia desde el principio
			adventurerActive = true;
			adventurerZ = -10.0f;
			adventurerX = 17.0f;
			adventurerRotY = 0.0f;
			adventurerTurned = false;
			adventurerInvisible = false;
